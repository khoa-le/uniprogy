...
class CFile extends CApplicationComponent
{
    ...
    {
        if ($filePath instanceOf CUploadedFile || trim($filePath)!='')
        {
            $uploaded = null;
            
            if ($filePath instanceOf CUploadedFile
            	|| (strpos($filePath, '\\')===false && strpos($filePath, '/')===false))
            {
                $uploaded = $filePath instanceOf CUploadedFile
                	? $filePath
                	: CUploadedFile::getInstanceByName($filePath);
                if ($uploaded)
                {
    ...
    private function pathInfo()
    {
        ...
        $this->_filename = isset($pathinfo['filename'])?$pathinfo['filename']:null;
